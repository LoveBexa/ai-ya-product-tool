...
function isMaskedStreamError(text: string): boolean {
  const t = text.trim()
  return t === GENERIC_STREAM_ERROR || t === "An error occurred"
}
...
function streamPartError(m: UIMessage): string | null {
  for (const part of m.parts ?? []) {
    const raw = part as { type?: string; errorText?: string }
    if (raw.type === "error" && raw.errorText?.trim()) {
...
export function DiscoveryChat() {
  ...
  const chatFailureMessage = useMemo(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      const partErr = streamPartError(messages[i])
      if (partErr) return formatAiError(partErr)
    }
    if (chatError) return formatAiError(chatError)
    return null
  }, [messages, chatError])
  ...
  const messageList = (
    <>
      ...
      {chatFailureMessage && (
        <p className="rounded-xl border border-warning/30 bg-warning/10 px-3 py-2.5 text-sm leading-relaxed text-alert-text">
          {chatFailureMessage}
        </p>
      )}
      {!onboarding && showAnalysisPreview && (
        <div className="flex justify-start">
  ...
  if (onboarding) {
    return (
      <div className="flex h-full min-h-0 flex-1 flex-col gap-2">
        {error && <p className="shrink-0 text-center text-xs text-alert-text">{error}</p>}
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div
            ref={scrollRef}
            className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6"
          >
            <div className="space-y-4 sm:space-y-5">{messageList}</div>
          </div>
        ...
        <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card">
          ...
          <div className="shrink-0 border-t border-border p-2.5 sm:p-3">
            ...
            <DiscoveryUploadZone
              ...
              disabled={busy}
            />
            <div className="mt-2">{chatInput}</div>
            </div>
          </div>
        </div>
